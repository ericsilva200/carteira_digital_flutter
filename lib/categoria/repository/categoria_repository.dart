...
class CategoriaPreferences {
  ...
  Future<List<Categoria>> findAll() async {
    ...
    return categoriasString.map((json) => Categoria.fromJsonString(json)).toList();
  }

  Future<List<Categoria>> findAllByUserId(userId) async {
    final prefs = await SharedPreferences.getInstance();
    final categoriasString = prefs.getStringList('categorias')?? [];
    return categoriasString.where((json) => Categoria.fromJsonString(json).userId == userId).map((json) => Categoria.fromJsonString(json)).toList();
  }

  Future<List<Categoria>> findAllByName(name) async { 
    final prefs = await SharedPreferences.getInstance();
    final categoriasString = prefs.getStringList('categorias')?? [];
    return categoriasString.where((json) => Categoria.fromJsonString(json).name == name).map((json) => Categoria.fromJsonString(json)).toList();
  ...
  Future<bool> save(Categoria categoria) async {
    ...
    if(catByName.length == 0){
      ...
      final categoriasString = categorias.map((c) => json.encode(c.toJson())).toList();
